...
class Loss(nn.Module):
    ...
    def forward_co_cluster(self,h1, h2):
        s1 = torch.matmul(h1,h1.T)
        s2 = torch.matmul(h2, h2.T)
        MSE = nn.MSELoss()
        return MSE(s1,s2)
    ...
    def forword_debiased_instance(self, h, h_i, y_pred):
        ...
        out = h
        neg = torch.exp(torch.mm(out, out.t().contiguous()) / temperature)    # neg = torch.exp(torch.mm(out, h_i.t().contiguous()) / temperature)??
